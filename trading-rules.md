...
layout: page
title: Trading Rules
subtitle: we suggest reading these before trading on KIWI Exchange
---

This page explains in detail the types of orders offered by KIWI Exchange, and how they behave, followed by the terms and conditions for trading on KIWI Exchange.

### Limit Orders

KIWI Exchange uses Limit Orders to ensure you get a fair price.
...
KIWI Exchange supports the following Order Terms for your orders, which let you control what happens when your order can or cannot be filled:
...
KIWI Exchange performs all matching using a smart contract running on the Ethereum blockchain. This is good - it means you don't need to worry about our servers failing, being shutdown or being hacked.
...
|Action|KIWI Exchange Fee|Ethereum Gas Amount|
...
KIWI Exchange runs entirely on the Ethereum blockchain. The nature of the blockchain means that thousands of Ethereum nodes around the world are busy working to collectively agree on the state of the blockchain - in particular, to agree on which orders are in the order book, and on the state of each order.
...
If this occurs for an order-related transaction then clients may see odd behaviour in the KIWI Exchange Web UI such as:
...
For now, we recommend clients mitigate against this risk by waiting several Ethereum blocks (e.g. 12 blocks) before treating a trade or order state as final. This is particularly important for clients arbitraging between KIWI Exchange and off-chain exchanges.
...
From time to time, we may release a new exchange contract for a trading pair and make the KIWI Exchange web UI point to the new contract. For example, we might do this to improve performance, add features, or adjust minimum order sizes.
...
The KIWI Exchange exchange contract has no notion of sign-up, approval, or indeed identity. We cannot and do not intend to police behaviour of clients of the exchange contract.
...
The KIWI Exchange Solidity Contracts, Web UI, and software libraries ("the Dapp") are provided "as is", without warranty of any kind, express or implied, including but not limited to the warranties of merchantability, fitness for a particular purpose and noninfringement.
...
The Dapp contracts will live as long as the Ethereum blockchain, but no warranty is given that the KIWI Exchange website will continue to provide access to the contracts.
